function [model, varargout] = LLiRaMLVQ_train(trainSet, trainLab, varargin)
...
p.addParameter('classwise',0, @(x)(~(x-floor(x))));
p.addParameter('dim',size(trainSet,2), @(x)(sum(~(x-floor(x)))==length(x) ));
p.addParameter('regularization',0, @(x)(isfloat(x) && sum(x>=0)==length(x)));
...
p.addParameter('learningRatePrototypes', [], @(x)(isfloat(x) || isa(x,'function_handle'))); %  && (length(x)==2 || length(x)==p.Results.epochs))
p.addParameter('learningRateMatrix', [], @(x)(isfloat(x) || isa(x,'function_handle')));
...
if ~isempty(p.Results.initialMatrices)
    error('matrix initializations with given not implemented!');
    if classwise
...
if initializeRandom
%         TODO: this could be done better! elseif more readable
    omega = rand(dim(1),nb_features)*2-ones(dim(1),nb_features);
    if classwise
        if length(dim)==1
    ...
    for i=1:length(dim)
        psis{i} = rand(dim(i),dim(i))*2-ones(dim(i),dim(i));
    end
end
...
omega = omega/sqrt(sum(arrayfun(@(d) sum(omega(:,d).*omega(:,d)),1:nb_features)));
psis = cellfun(@(x) x/sqrt( sum(arrayfun(@(d) sum(x(:,d).*x(:,d)),1:size(x,2))) ),psis,'uni',0);
...
case{'fminlbfgs'}
    ...
    model = struct('w',w,'c_w',c_w,'omega',omega,'psis',{psis});
    clear w c_w psis;
% save('test','model','nb_ppc')    
    variables = [model.c_w,nan(sum(nb_ppc),nb_features)];
%     variables = cat(2,cell2mat(arrayfun(@(x) bsxfun(@times,model.c_w(x),ones(nb_ppc(x),1)),1:length(nb_ppc),'uni',0)'),nan(sum(nb_ppc),nb_features));
    variables(:,2:end) = model.w;
    variables(end+1:end+size(omega,1),1:end) = [nan(size(omega,1),1),omega];
    mats = cell2mat(arrayfun(@(x) [nan(dim(x),1),model.psis{x}],1:length(dim),'uni',0)');
    variables(end+1:end+sum(dim),:) = [mats,nan(sum(dim),size(variables,2)-size(mats,2))];

    LabelEqualsPrototype = bsxfun(@eq,trainLab,variables(~isnan(variables(:,1)),1)');
    useFun = @(a,b,c,d,e,f,g) LLiRaMLVQ_optfun(a,b,c,d,e,f,g);
    [variables,fval,ExitFlag] = fminlbfgs(@(variables) useFun(variables,trainSet,LabelEqualsPrototype,regularization,1,1,dim),variables,options);
%     % learn prototypes without relevances
%     [variables,fval,ExitFlag] = fminlbfgs(@(variables) useFun(variables,trainSet,LabelEqualsPrototype,regularization,1,0,dim),variables,options);
%     % learn relevances without the prototypes
%     [variables,fval,ExitFlag] = fminlbfgs(@(variables) useFun(variables,trainSet,LabelEqualsPrototype,regularization,0,1,dim),variables,options);
